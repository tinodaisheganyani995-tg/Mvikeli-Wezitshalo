...
# - One Gunicorn worker recommended
# - Gatekeeper is loaded only during gatekeeper inference
# - ReportLab is lazy-loaded only for PDF generation
# - scikit-learn is lazy-loaded only for evaluation
# - Grad-CAM hooks are created only when needed
# - inference_mode() is used for normal inference
# - temporary tensors/models are explicitly released
...
# Protect the server from very large image uploads.
...
# Helps keep CPU memory/threads under control on small servers.
try:
...
GATEKEEPER_PATH = "best_gatekeeper_resnet18.pth"
...
# =========================================================
# 8. EXTERNAL VALIDATION DATASET
# =========================================================

EXTERNAL_VALIDATION_DIR = "external_validation"


# =========================================================
# 9. ALLOWED IMAGE EXTENSIONS
# =========================================================
...
# =========================================================
# 10. DISEASE DISPLAY NAME HELPER
# =========================================================
...
def load_checkpoint_into_model(
    ...
    try:
        ...
        # The checkpoint is no longer needed after loading.
        del checkpoint
...
# =========================================================
# 14. GATEKEEPER LOADER
#
# IMPORTANT:
# The gatekeeper is NOT kept permanently in memory.
# It is loaded only while a prediction is being made.
# =========================================================
...
def load_gatekeeper():
    ...
    gatekeeper = load_checkpoint_into_model(
        gatekeeper,
        GATEKEEPER_PATH
    )
...
def check_image_quality(image_path):
    ...
    # Release OpenCV arrays before returning.
    del gray
    del image
...
# =========================================================
# 18. CREATE GRAD-CAM IMAGE
#
# Grad-CAM is created only when needed.
# No permanent hooks are kept in memory.
# =========================================================
...
def predict(image_path):
    ...
    try:
        ...
        for probability, index in top3_data:
...
def prediction():
    ...
    if not quality["valid"]:
        ...
        return render_template(
            ...
            prediction="Image Quality Problem",
            confidence=0.0,
            ...
            top3=[],
            is_uncertain=True,
            uncertainty_message=quality_message,
    ...
    if gate_result == "not_tomato":
        ...
        return render_template(
            ...
            image=filepath,
            image_filename=unique_filename,
            prediction="Tomato Leaf Not Detected",
    ...
    top2_confidence = result["top2_confidence"]
    ...
    try:
        ...
        # Do not let explainability failure
        # destroy the main prediction.
...
# 25. PDF REPORT
#
# ReportLab is imported ONLY when the user requests
# a PDF. This reduces startup memory.
# =========================================================
...
def download_report():
    ...
    display_name = latest_result.get(
        "display_name",
        get_display_name(disease)
    )

    confidence = latest_result["confidence"]

    image_path = latest_result["image"]

    disease_info = latest_result["disease_info"]

    top3 = latest_result["top3"]
...
#
# scikit-learn is imported only when evaluation is used.
# =========================================================
...
def evaluate_external_dataset(
    ...
    # COLORMAP_BLU may not exist in some OpenCV builds.
    # TURBO is widely available and gives a readable matrix.
